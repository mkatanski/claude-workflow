		...
		Stop: [
			{
				matcher: "",
				hooks: [
					{ type: "command", command: createStopHookCommand(projectPath) },
				],
			},
		],
		SessionEnd: [
			{
				matcher: "",
				hooks: [
					{ type: "command", command: createExitHookCommand(projectPath) },
				],
			},
		],
	};
...
export function installHooks(projectPath: string): void {
	...
	console.log(
		"These hooks will signal task completion to the workflow runner.",
	);
}
