...
describe('BreakpointManager', () => {
	...
	describe('Breakpoint Hit Callback', () => {
		...
		it('should handle logpoints without blocking', () => {
			...
			expect(hit).not.toBeNull();
			expect(hit?.breakpoint.logMessage).toBe('At node {node} with count {count}');
		});
	});
	...
	describe('Lifecycle', () => {
		...
		it('should throw error when used after disposal', () => {
			...
			expect(() => manager.add(createNodeBreakpoint('node1', 'before'))).toThrow(
				'BreakpointManager has been disposed'
			);
		});
