...
export class WorkflowRunner {
		...
		try {
			...
			for (let i = 0; i < this.steps.length; i++) {
				...
				if (step.outputVar && result.output !== undefined) {
					...
					if (this.verbose) {
						console.log(
							`Set ${step.outputVar} = ${result.output.slice(0, 100)}...`,
						);
					}
				}
				...
				if (result.gotoStep) {
					const targetIndex = this.steps.findIndex(
						(s) => s.name === result.gotoStep,
					);
					if (targetIndex === -1) {
						return {
							success: false,
							error: `Goto target not found: ${result.gotoStep}`,
						};
					}
	 ...
	 */
	private async executeStep(step: StepConfig): Promise<{
		success: boolean;
		output?: string;
		error?: string;
		gotoStep?: string;
