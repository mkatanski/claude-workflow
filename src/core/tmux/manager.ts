...
export class TmuxManager {
	...
	private externalizePromptToFile(prompt: string): string {
		...
		const promptFile = join(
			this.tempDir,
			`prompt_${randomUUID().slice(0, 8)}.md`,
		);
	 ...
	 */
	private buildClaudeCommand(prompt?: string, modelOverride?: string): string {
	...
	async launchClaudePane(
		...
		if (prompt.length > MAX_PROMPT_LENGTH) {
			...
			console.error(
				`Maximum allowed: ${MAX_PROMPT_LENGTH.toLocaleString()} chars`,
			);
			throw new Error(
				`Prompt too large (${prompt.length.toLocaleString()} chars). ` +
		...
		if (!paneId) {
			...
			throw new Error(
				`Failed to create tmux pane. tmux error: ${error ?? "unknown"}`,
			);
		}
	...
	async launchBashPane(command: string, cwd?: string): Promise<string> {
		const workingDir =
			cwd ?? this.claudeConfig.cwd ?? resolve(this.projectPath);
	...
	async closePane(): Promise<void> {
		...
		try {
			...
			await runCommand(["tmux", "send-keys", "-t", paneToClose, "C-c"]).catch(
				() => {},
			);
			await Bun.sleep(300);
