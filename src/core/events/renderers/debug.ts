...
export class DebugRenderer extends BaseRenderer {
  ...
  private renderBreakpointHit(event: DebugBreakpointHitEvent): void {
    const { breakpointId, nodeName, condition, hitCount, variables } = event.payload;
  ...
  private renderStepBefore(event: DebugStepBeforeEvent): void {
    ...
    if (this.debugConfig.showVariables && this.config.verbose && Object.keys(variables).length > 0) {
      this.writeLine('   Variables:');
      this.renderVariables(variables, '      ');
    }
  }
  ...
  private renderStepAfter(event: DebugStepAfterEvent): void {
    ...
    this.writeLine(`✓ Step ${stepType} completed: ${nodeName} (${this.formatDuration(duration)})`);

    if (this.debugConfig.showVariables && Object.keys(variableChanges).length > 0) {
  ...
  private renderExecutionResume(event: DebugExecutionResumeEvent): void {
    ...
    this.writeLine(`▶️  Resumed (${resumeMode}): ${nodeName} (paused for ${this.formatDuration(duration)})`);
    this.writeLine('');
    this.isPaused = false;
  }
   ...
   */
  private renderVariables(variables: Record<string, unknown>, indent: string = ''): void {
  ...
  private formatValue(value: unknown): string {
    ...
    if (type === 'object') {
      ...
      const entries = keys.map(k => `${k}: ${this.formatValue((value as Record<string, unknown>)[k])}`);
      return `{ ${entries.join(', ')} }`;
    }
  ...
  private promptForCommand(): void {
    ...
    this.writeLine('Commands: (c)ontinue, (s)tep-over, step-(i)n, step-(o)ut, in(s)pect <var>, (q)uit');
    this.rl.prompt();
